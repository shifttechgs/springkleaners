...
- (2026-07-09) **Business owner correction**: 6 of the 10 custom-quote-only service pages shipped 2026-07-08 were services the business doesn't actually offer — Mattress Cleaning, Retail & Shop Cleaning, Medical & Clinic Cleaning, Restaurant & Commercial Kitchen Cleaning, School & Educational Facility Cleaning, and Pressure Washing / Exterior Cleaning. Removed via a new migration (`2026_07_09_140344_remove_unoffered_custom_quote_services`, symmetric down() re-inserts if ever needed), their `config/service_pages.php` entries deleted, `StructuredDataTest::customQuoteServices()` trimmed to the 4 real remaining ones (Spring Cleaning, Oven Deep Cleaning, Fridge & Appliance Cleaning, Blind & Curtain Cleaning), and the one cross-link to `retail-shop-cleaning` in the office-cleaning-cost blog post removed. `Services::list()`/sitemap/"Other services" cross-sell are all DB-driven so no other template changes were needed. **14 services now live** (was 20). Don't re-add these without the business owner confirming they're actually offered.
