...
<div class="wrap">
    ...
    <table class="hdr">
        <tr>
            <td width="46%">
                ...
                <div class="meta-val">{{ $booking->invoice_number }}</div>
                <div class="meta-lbl">Date Issued</div>
                <div class="meta-val">{{ ($booking->invoiced_at ?? now())->format('d M Y') }}</div>
            ...
            <td width="54%" align="right">
                ...
                <div class="co">
                    ...
                    @foreach (\App\Support\Company::addressLines() as $line)
                        {{ $line }}<br>
                    @endforeach
                    Reg No: {{ \App\Support\Company::regNo() }}<br>
    ...
    <table class="totals">
        ...
        @if ($depositPaid > 0)
        <tr class="sub">
            <td class="lc">Less: Deposit Paid</td>
            <td>&minus;R&nbsp;{{ number_format($depositPaid, 2) }}</td>
        </tr>
        @endif
        <tr class="grand">
            <td class="lc">Balance Due</td>
            <td>R&nbsp;{{ number_format($balanceDue, 2) }}</td>
        </tr>
    </table>
    ...
    <div class="paid-stamp">
        PAID in full on {{ $booking->paid_at?->format('d M Y') }}
        @if ($booking->payment_method) &middot; {{ ucfirst($booking->payment_method) }} @endif
    </div>
    @else
    ...
    <div class="card-wrap">
        <div class="notes-card">
            Questions about this invoice? Contact us on {{ \App\Support\Company::cell() }} or {{ \App\Support\Company::email() }}.
        </div>
    </div>
