...
<head>
    ...
    <title>Accept Invite | SpringKleaners</title>
...
<body class="min-h-screen flex items-center justify-center px-4" style="background: radial-gradient(120% 120% at 50% -10%, #0d2a4a 0%, #081d3a 45%, #040f1f 100%);">
    ...
    <div class="w-full max-w-[380px] relative z-10">
        ...
        <div class="bg-white rounded-2xl p-8" style="box-shadow: 0 20px 50px -12px rgba(0,0,0,0.4);">
            <p class="text-[#8a94a6] text-[11px] uppercase tracking-wider font-semibold mb-1.5">Welcome, {{ $invitee->name }}</p>
            <h1 class="text-[#081d3a] font-extrabold text-[20px] tracking-tight mb-2">Set your password</h1>
            <p class="text-[#647082] text-[13px] mb-6">You've been invited as <strong class="text-[#081d3a]">{{ $invitee->role->label() }}</strong>. Choose a password to activate your account.</p>
            ...
            <form method="POST" action="{{ url()->full() }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="email" value="{{ $invitee->email }}" disabled
                           class="w-full bg-[#f6f7fa] border border-[#e8eaf0] rounded-xl px-4 py-3 text-[#8a94a6] text-[14px]">
                </div>
                <div>
                    <label class="block text-[#8a94a6] text-[11px] uppercase tracking-wider mb-1.5 font-semibold">Password</label>
