...
<head>
    ...
    <title>Forgot Password | SpringKleaners</title>
...
<body class="min-h-screen flex items-center justify-center px-4" style="background: radial-gradient(120% 120% at 50% -10%, #0d2a4a 0%, #081d3a 45%, #040f1f 100%);">
    ...
    <div class="w-full max-w-[380px] relative z-10">
        ...
        <div class="bg-white rounded-2xl p-8" style="box-shadow: 0 20px 50px -12px rgba(0,0,0,0.4);">
            ...
            <h1 class="text-[#081d3a] font-extrabold text-[20px] tracking-tight mb-2">Forgot your password?</h1>
            <p class="text-[#647082] text-[13px] mb-6">Enter your email and we'll send you a link to reset it.</p>

            @if (session('status'))
                <div class="mb-5 px-3.5 py-3 bg-emerald-50 border border-emerald-100 rounded-xl text-emerald-700 text-[13px] font-medium">
                    {{ session('status') }}
                </div>
            @endif
